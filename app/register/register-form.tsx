...
import { CONCEPT, type ConceptLocale } from "./concept";

export default function RegisterForm({ redirect = "/", brand = "Avrupa Uyanış Hizmetleri" }: { redirect?: string; brand?: string }) {
  const router = useRouter();
  const { copy } = useInvenimusCopy();
  const a = copy.auth;
  const locale = useClientLocale() as ConceptLocale;
  const cc = CONCEPT[locale]?.register ?? CONCEPT.tr.register;
    ...
    <div className="flex min-h-screen flex-col bg-ceyhun-cream font-sans text-ceyhun-ink lg:flex-row">
      ...
      <div className="relative hidden overflow-hidden bg-ceyhun-night text-ceyhun-cream lg:flex lg:w-1/2">
        <div className="pointer-events-none absolute inset-0">
          <div className="absolute -left-20 bottom-10 h-[28rem] w-[28rem] rounded-full bg-ceyhun-wine/25 blur-[120px]" />
          <div className="absolute -right-16 top-10 h-[26rem] w-[26rem] rounded-full bg-ceyhun-gold/20 blur-[130px]" />
      ...
      <div className="relative flex flex-1 items-center justify-center px-4 py-10 sm:px-8">
        ...
        <div className="w-full max-w-sm">
          ...
          <h1 className="font-syne text-3xl font-extrabold tracking-tight">{a.register.formTitle}</h1>
          <p className="mt-2 text-sm text-ceyhun-slate">{a.register.formSubtitle}</p>
          ...
          <form onSubmit={submit} className="mt-8 flex flex-col gap-4">
            ...
            <p className="text-center text-[11px] text-ceyhun-ink/40">{a.register.terms}</p>
          </form>

          <p className="mt-6 text-center text-sm text-ceyhun-slate">
